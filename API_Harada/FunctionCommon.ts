...
import { Model_customer } from "./CDSModel/Model_customer";
import { PowerApps_TableName_Dev, PowerApps_TableName, CONST_PRODUCT_ID, CONST_OptionKeyStyle_key, NAME_FILE_SVG, CONST_ColorTransparent, INFO_ITEM_OUTPUT, SIZE_OUTPUT_PATTERN, OptionKeySize, SIZE_MODEL_PATTERN, INFO_ITEM_OUTPUT_DEV, SIZE_OUTPUT_PATTERN_DEV } from "./Constants";
 ...
 * @returns
 */
export const F_ConvertToJSONObj = (data: string) => {
    let obj;
    obj = JSON.parse(data);
    return obj;
}

export const F_ConvertObjectToJSONString = (obj: any) => {
    let str;
    str = JSON.stringify(obj);
    return str;
}

export const F_Log = (obj: any, obj2: any = undefined) => {
    if (obj2 === undefined) {
...
export const F_LogError = (obj: any, obj2: any = undefined) => {
    if (obj2 === undefined) {
...
export const F_UnsetKeys = (obj: any, arrRemoveKeys: any) => {
    var target: any = {};
    for (var i in obj) {
        if (arrRemoveKeys.indexOf(i) >= 0) continue;
...
export const F_UnsetKey = (obj: any, removeKey: any) => {
    var target: any = {};
    for (var i in obj) {
        if (removeKey == i) continue;
        if (!Object.prototype.hasOwnProperty.call(obj, i)) continue;
        target[i] = obj[i];
    }
    return target;
...
export const F_GetValueByIndex = (obj: any, key: any) => {
    for (var i in obj) {
...
export const F_PaddingLeft = (str: string, padChar: string, len: number) => {
    while (str.length < len)
        str = padChar + str;
    return str;
}

export const F_PaddingRight = (str: string, padChar: string, len: number) => {
    while (str.length < len)
        str = str + padChar;
    return str;
}
 ...
 * @returns
 */
export const F_ConvertBufferToStream = (buffer: any) => {
...
export const F_ConvertURLToReadStream = (url: any) => {
    const stream = fs.createReadStream(url);
    return stream;
}

export const F_ConvertReadStreamToBuffer = (readStream: any) => {

    const buff = Buffer.from(readStream);
    return buff;
}

export async function F_GetJson(queryStr: string) {
    F_Log('F_GetJson');
    let dataRS = '';
    await fetch(queryStr).then(async (response: any) => {
...
export const ConvertURLToBase64Image = async (urlImage: string) => {
    const type = urlImage.split('.').pop();
    const image = await axios.get(urlImage, {responseType: 'arraybuffer'});
...
export const F_GetFileFromURL = (file: any) => {
    return new Promise<any>(resolve => {
        request.get(file.url, function (error: any, response: any, body: any) {
...
export const F_IsNullOrEmpty = (str: string) => {
    if (str == null
        || str == undefined
        || str == '') {
        return true;
...
export const F_IsNullOrEmptyListItem = (obj: any) => {
    if (obj == null
        || obj == undefined) {
        return true;
    }
...
export const F_GetValueByKey = (obj: any, key: any) => {
    if (!F_IsNullOrEmptyListItem(obj)) {
        for (var i in obj) {
            if (key == i && Object.prototype.hasOwnProperty.call(obj, i)) {
                return obj[i];
            }
...
export const F_IsNullOrEmptyAttribute = (obj: any, attribute: any) => {
    return F_GetValueByKey(obj, attribute) == undefined;
}

export const F_RoundNumber = (numberInput: number, decimalPlaces: number) => {
    var temp = Math.pow(10, decimalPlaces);
    return Math.round(numberInput * temp) / temp;
}

export const F_GetFileNameFromPath = (filePath: string) => {
    var fileName = filePath.replace(/^.*[\\\/]/, '');
    return fileName;
}

// Output ex: '2012-11-04 14:55:45'
export const F_GetCurrentDateTimeYYYY_MM_DD_HH_MM_SS = () => {
 ...
 * @returns
 */
export const F_GetCurrentDateTimeYYYYMMDDHHMMSS_FF = () => {
 ...
 */
export const F_CreateFolderIfNotExist = (folderPath: string) => {
 ...
 * @returns
 */
export const F_FormatCurrency = (input: number) => {
 ...
 * @returns
 */
export const F_GetBtoaFromShopify = (_gid: string) => {
 ...
 *    }
 */
export const F_ConvertBase64ImageToStream = (base64Image: any) => {
    try {
        if (base64Image !== '') {
            ...
            let stream = F_ConvertBufferToStream(buffer);
 ...
 *    }
 */
 export const F_ConvertStringToStream = (jsonData: any) => {
    try {
        if (jsonData !== '') {
            // Get Mime type use this one
 ...
 */
export const waitForOneSecond = () => {
    return new Promise(resolve => {
        resolve('Resolving!');
    });
}

export const ModeIsDev = (env: string) => {
    if (env && env === 'dev') {
        Model_customer.mTableName = PowerApps_TableName_Dev.customer;
    } else {
        Model_customer.mTableName = PowerApps_TableName.customer;
    }
}
//#endregion
 ...
 * @returns
 */
export const ConvertImageHTMLToFabric = (url: string) => {
    return new Promise(async (resolve, reject) => {

        const fabric = require('fabric').fabric;
        fabric.util.loadImage(url, function(img:any) {
 ...
 * @returns
 */
export const loadFromJSON = (canvas:any, json: any) => {
    return new Promise((resolve, reject) => {
 ...
 * @returns
 */
export const enlivenObjects = async (canvas:any, objects: any) => {
    return new Promise((resolve, reject) => {
        
        const fabric = require('fabric').fabric;
        // EnlivenObjects([array of json objects], [callback])
        fabric.util.enlivenObjects(objects, (objs:any) => {
            objs.forEach(async function (item: any) {
                item.dirty = true;
                canvas.add(item);
            });
 ...
 * @param {*} jsonCanvas
 */
export const UpdatePropertyForObjects = (jsonCanvas: any) => {

    return new Promise((resolve, reject) => {

        jsonCanvas.objects.map(async (item:any) => {
            if (item.type === 'text') {
 ...
 * @returns
 */
export const ConvertImageHTMLToFabricDefault = (base64Image: string) => {
    return new Promise(async (resolve, reject) => {

        const fabric = require('fabric').fabric;
        fabric.util.loadImage(base64Image, function(img: any) {
            // const img = await loadImage(base64Image);
            let newImageFabric: any = new fabric.Image(img);
            newImageFabric.set({
                angle: 0,
                padding: 0,
                cornersize: 0,
                ...
                visible: true,
                hoverCursor: 'default',
                name: "object_default" + F_CreateGUID(),
 ...
 * @returns
 */
export const ConvertSvgStringToFabric = (svgStr: any, name: string, text: string, isReadOnly: boolean, customContrlType: string) => {
    return new Promise((resolve, reject) => {
        const group: any[] = [];
        const fabric = require('fabric').fabric;
        fabric.loadSVGFromString(svgStr, async function (objects: any, options: any) {
                var loadedObjects: any = new fabric.Group(group);
                loadedObjects.set({
                    // objectCaching: false,
                    dirty: true,
 ...
 * @returns
 */
const ConvertImageHTMLToFabric_02 = (imageHTMLObj: any, fileName: string, name: string, top: number, left: number, 
                                    scaleX: number, scaleY: number, imageFitType: string, customContrlType: string, 
                                    note: string, isReadOnly: boolean, visible: boolean = true) => {
    const fabric = require('fabric').fabric;
 ...
 * @param {boolean} isReadOnly
 */
export const F_SetReadonlyFabricObject = (fabricObj: any, isReadOnly: boolean) => {
    fabricObj.set({
        selectable: isReadOnly ? false : true,
        evented: isReadOnly ? false : true,
    });
 ...
 * @returns
 */
export const ConvertBase64ToImageHTML_02 = (url: string) => {
    return new Promise(async (resolve, reject) => {
        const fabric = require('fabric').fabric;
        fabric.util.loadImage(url, function(img: any) {
            resolve(img);
 ...
 */
export const RegisterFontOnCanvas = () => {
 ...
 * @returns
 */
export const ProductIsPolo = (productId: string) => {
    return productId === CONST_PRODUCT_ID.POLO ? true : false;
}
 ...
 * @returns
 */
export const ReadFileFromUrl = async (filename: string) => {
    let rs = '';
    const fs = require("fs");
    rs = await fs.promises.readFile(filename, 'utf8')
    return rs;
}
 ...
 */
export const SetSizeCanvas = (canvas: any, widthCanvas: number, heightCanvas: number) => {
    canvas.backgroundColor = CONST_ColorTransparent;
 ...
 * @param {*} canvas
 */
export const RenderRulerOnOutputPattern = (productId: string, canvas: any) =>
{
    const isPolo = productId === CONST_PRODUCT_ID.POLO ? true : false;
    const isShirts = productId === CONST_PRODUCT_ID.SHIRTS ? true : false;
 ...
 * @param {*} fabricObj
 */
export const SetFabricFullCanvasWidthHeight = (canvas: any, fabricObj: any) =>
{
 ...
 * @returns
 */
export const GetObjectInArrayByName = (array: any[], name: string) => {
 ...
 * @returns
 */
export const ConvertBase64ToImageHTML = (base64Image: string) => {
    return new Promise((resolve, reject) => {
        const img = new Image();
 ...
 * @returns
 */
export const GetListUrlItemCropModel = (productId: string, styleCode: string, environment: string) =>
{
    const arrItemsPatternSVG: any[] = [];

    const isPolo = productId === CONST_PRODUCT_ID.POLO ? true : false;
    const isShirts = productId === CONST_PRODUCT_ID.SHIRTS ? true : false;

    const isMen = styleCode === CONST_OptionKeyStyle_key.C1 ? true : false;
    const isLadies = styleCode === CONST_OptionKeyStyle_key.C2 ? true : false;

    const arrNameItems = isPolo ? NAME_FILE_SVG.POLO : (isShirts ? NAME_FILE_SVG.SHIRTS : NAME_FILE_SVG.HOODIE);
    ...
    arrNameItems.forEach((item: string) => arrItemsPatternSVG.push(`${ folderUrl }${ item }.svg`));
 ...
 * @returns
 */
export const GetArrayUrlItemSvgOutput = (productId: string, styleCode: string, size: string, environment: string) =>
{
    const arrItemsPattern: any[] = [];
    ...
    arrNameItems.forEach((item: string) => arrItemsPattern.push(`${ folderUrl }${ item }.svg`));
 ...
 * @returns
 */
const GetImageBoundWithoutTransparentPixel = (ctx: CanvasRenderingContext2D, wCanvas: number, hCanvas: number) => {
    let rs = {
        top: 0,
        right: 0,
        bottom: 0,
 ...
 * @returns
 */
const CropImageWithoutTransparentPixel = (canvas: any, imageFabric: any, cropDelta: number = 0, backgroundColor: string = '') => {
    return new Promise(async (resolve, reject) => {
        if (imageFabric !== null) {
            let arrBKVisible: any[] = [];
            ...
            for (let i = 0; i < objs.length; i++) {
                if (objs[i].name != imageFabric.name) {
                    objs[i].set({
                        visible: GetObjectInArrayByName(arrBKVisible, objs[i].name).visible,
                    });
                }
            }
            ...
            const resultTran: any = await CropTransparentPixel(imageSrc, cropDelta);
            const imageHTMLObj = await loadImage(resultTran.base64);
            resolve(
                        { 
 ...
 * @returns
 */
const CropTransparentPixel = (imageSrc: any, cropDelta: number = 0) => {
    return new Promise(async (resolve, reject) => {
 ...
 * @returns
 */
const AddImageToCanvasByFitType = async (borderObj: any, canvas: any, imageHTMLObj: any, fileName: string, imageFitType: string, customContrlType: string, note: string = '') => {
    switch (imageFitType) {
        // case '1':
        //     return await AddImageFitTypeFillOrCenter(props, canvas, imageHTMLObj, fileName, imageFitType, customContrlType, note);

        case '3':
            return await AddImageFitTypeFillOrCenter(borderObj, canvas, imageHTMLObj, fileName, imageFitType, customContrlType, note);
 ...
 * @returns
 */
const AddImageFitTypeFillOrCenter = async (borderObj: any, canvas: any, imageHTMLObj: any, fileName: string, imageFitType: string, customContrlType: string, note: string = '') => {
    ...
    const imageFabric = await ConvertImageHTMLToFabric_02(imageHTMLObj, fileName, /*name*/ '8888', top, left, scaleX, scaleY, '3', customContrlType, note, false);
 ...
 * @returns
 */
export const CropItemOutputPattern = (canvas2:any, imgFabric:any, imgFabricRight:any, productId: string, styleCode: string, environment: string) => {
    return new Promise(async (resolve, reject) => {
        let style = styleCode;
        let arrItemsPattern: any[] = [];

        //#region Get array name of file SVG
        const arrItemsPatternSVG: string[] = GetListUrlItemCropModel(productId, styleCode, environment);
        //#endregion

        // 2022.12.15
        if (environment === '') {
            F_SetCanvasWidthHeightByProductStyle(canvas2, productId, style);
        } else if (environment === 'dev') {
            F_SetCanvasWidthHeightByProductStyle_Dev(canvas2, productId, style);
        }

        SetFabricFullCanvasWidthHeight(canvas2, imgFabric);
        if (productId === CONST_PRODUCT_ID.SHIRTS) {
            SetFabricFullCanvasWidthHeight(canvas2, imgFabricRight);
        }
        ...
        for (let strURI of arrItemsPatternSVG) {
            ...
            const svgStringVal = await ReadFileFromUrl(strURI);
            ...
            const shape:any = await ConvertSvgStringToFabric(svgStringVal, '', '', false, 'SVG');
            SetFabricFullCanvasWidthHeight(canvas2, shape);
            let newFabricObj: any = await CropImageByShape(canvas0, canvas2, imgFabricCrop, shape);
            ...
            if (productId !== CONST_PRODUCT_ID.HOODIE) { newFabricObj.rotate(90); }
 ...
 * @returns
 */
export const AddShapeToCanvas = async (borderObj: any, canvas: any, svgStringVal: string, shapeId: string) => {

    const loadedObjects: any = await ConvertSvgStringToFabric(svgStringVal, '999' /*OBJECT_NAME_PREFIX + mObjectNameAutoIncrease*/, shapeId, false, 'SVG');
 ...
 * @returns
 */
export const CropImageByShape = (canvas0: any, canvas2: any, imageFabric: any, shapeObj: any) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (imageFabric !== undefined && shapeObj !== undefined) {
            ...
            // SVG
            const svgGroup: any = await AddShapeToCanvas(borderObj, canvas2, shapeObj.svgStr, 'shapeName');
            svgGroup.set({
            ...
            let newImageFabric: any = await ConvertImageHTMLToFabric(imageSrc2);
            newImageFabric.set({
                // objectCaching: false,
                angle: 0,
                padding: 0,
            ...
            temp2._objects.map((item: any) => { item.fill = 'black'; });
            ...
            canvas2.getObjects().map((obj: any) => {
                obj.setCoords();
            });
            ...
            const cropWithout: any = await CropImageWithoutTransparentPixel(canvas2, newImageFabric, cropDelta, CONST_ColorTransparent);
            const imgHTML = cropWithout.imageHTMLObj;
            const rec = cropWithout.rec;

            canvas2.clear();

            const imageFabricAdded = await AddImageToCanvasByFitType(borderObj, canvas2, imgHTML, imageFabric.customFileName, '3', 'Image');
 ...
 * @returns
 */
export const CreateOutputPatternNewVersion = async (canvas3: any, product_id: string, styleCode: string, arrItemOutput: any, size: any, base64Ruler: any, environment: string) => {
    ...
    let base64Pattern;
    if (size === "LL") {
        base64Pattern = await CreateOutputPattern(canvas3, product_id, styleCode, arrItemOutput, size, base64Ruler, environment);
        return base64Pattern;
    } else {

        const infoRes = (environment === '') ? GetInfoOutputPattern(product_id, styleCode, size) : GetInfoOutputPattern_Dev(product_id, styleCode, size);
        const arrInfoItem: any[][] = infoRes.InfoItem;
        ...
        SetSizeCanvas(canvas3, wPattern, hPattern);

        const arrUrlItem: string[] = GetArrayUrlItemSvgOutput(product_id, styleCode, size, environment);
        // console.log('______(02)_GetArrayUrlItemSvgOutput__________', arrUrlItem);
        const arrNewFabric: any[] = [];
        ...
        for (let i = 0; i < arrInfoItem.length; i++) {
            ...
            arrItem[i].set({
                ...
                scaleX: infoScaleX,
                scaleY: infoScaleY,
                angle: product_id !== CONST_PRODUCT_ID.HOODIE ? 90 : 0,
            });
            canvas3.add(arrItem[i]);
            canvas3.renderAll();

            const base64ImageData = canvas3.toDataURL();
            const imageRepeat = await ConvertImageHTMLToFabricDefault(base64ImageData);

            SetFabricFullCanvasWidthHeight(canvas3, imageRepeat);

            const strURI = arrUrlItem[i];
            const svgStringVal = await ReadFileFromUrl(strURI);
            // const name = strURI.substring(strURI.lastIndexOf("/") + 1, strURI.lastIndexOf("."));
            const name = arrItem[i].customFileName;
            const shape = await ConvertSvgStringToFabric(svgStringVal, '', name, false, 'SVG');
            SetFabricFullCanvasWidthHeight(canvas3, shape);

            const newFabricObj = await CropItemByShape(imageRepeat, shape);
            newFabricObj.set({
                customFileName: name,
            })
        ...
        // 2022.12.02
        if (base64Ruler !== '') {
            const fbRuler = await ConvertImageHTMLToFabricDefault(base64Ruler);
            canvas3.add(fbRuler);
        }
 ...
 * @returns
 */
export const CreateOutputPattern = async (canvas3: any, product_id: string, style_code: string, arrItemOutput: any, size: any, base64Ruler: string, environment: string) => {
    
    canvas3.clear();
    const productId = product_id;
    const style = style_code;
    const infoRes = (environment === '') ? GetInfoOutputPattern(productId, style, size) : GetInfoOutputPattern_Dev(productId, style, size);
    const arrItem = arrItemOutput;
    ...
    canvas3.backgroundColor = CONST_ColorTransparent;
 ...
 * @returns
 */
export const CropItemByShape = (imageFabric: any, shapeObj: any) =>
{
    if (imageFabric && shapeObj) {
        let newImageFabric = imageFabric;
        let shapeCrop = shapeObj;
        shapeCrop._objects.map((item: any) => { item.fill = 'black'; });
        shapeCrop.set({
 ...
 * @param {string} style
 */
export const F_SetCanvasWidthHeightByProductStyle = (canvas: any, productId: string, style: string) =>
{
    const sizePattern = GetSizePatternModel(productId, style);
 ...
 * @returns
 */
export const GetInfoOutputPattern = (productId: string, styleCode: string, size: string) =>
{
    let infoItem: any;
    ...
    const productNameItem = isPolo ? INFO_ITEM_OUTPUT.POLO : (isShirts ? INFO_ITEM_OUTPUT.SHIRTS : INFO_ITEM_OUTPUT.HOODIE);
    const productNameSize = isPolo ? SIZE_OUTPUT_PATTERN.POLO : (isShirts ? SIZE_OUTPUT_PATTERN.SHIRTS : SIZE_OUTPUT_PATTERN.HOODIE);
 ...
 * @param {string} style
 */
export const F_SetCanvasWidthHeightByProductStyle_Dev = (canvas: any, productId: string, style: string) =>
{
    const sizePattern = GetSizePatternModel(productId, style);
 ...
 */
export const GetSizePatternModel = (productId: string, styleCode: string) =>
{
    const isPolo = productId === CONST_PRODUCT_ID.POLO ? true : false;
    const isShirts = productId === CONST_PRODUCT_ID.SHIRTS ? true : false;
    // const isHoodie = productId === CONST_PRODUCT_ID.HOODIE ? true : false;
    ...
    const sizeProduct = isPolo ? SIZE_MODEL_PATTERN.POLO : (isShirts ? SIZE_MODEL_PATTERN.SHIRTS : SIZE_MODEL_PATTERN.HOODIE);
 ...
 * @returns
 */
export const GetInfoOutputPattern_Dev = (productId: string, styleCode: string, size: string) =>
{
    let infoItem: any;
    let infoSize = { Width: 0, Height: 0 };
